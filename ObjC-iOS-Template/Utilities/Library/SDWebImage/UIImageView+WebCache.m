...
- (void)sd_setImageWithURL:(NSURL *)url placeholderImage:(UIImage *)placeholder cropImageWithSize:(CGSize)size{
    [self cancelCurrentImageLoad];
    objc_setAssociatedObject(self, &imageURLKey, url, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    self.image = [Utility imageWithImage:placeholder scaledToSize:size];
    if (url) {
        __weak UIImageView *wself = self;
        id <SDWebImageOperation> operation = [SDWebImageManager.sharedManager downloadImageWithURL:url options:SDWebImageRefreshCached progress:nil completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, BOOL finished, NSURL *imageURL) {            
            if (!wself) return;
            dispatch_main_sync_safe(^{
                if (!wself) return;
                if (image) {
                    wself.image = [Utility imageWithImage:image scaledToSize:size];
                    [wself setNeedsLayout];
                } else {
                    if ((1 & SDWebImageDelayPlaceholder)) {
                        wself.image = [Utility imageWithImage:placeholder scaledToSize:size];
                        [wself setNeedsLayout];
                    }
                }
            });
