...
- (void)sd_setImageWithURL:(NSURL *)url forState:(UIControlState)state placeholderImage:(UIImage *)placeholder cropToSize:(CGSize)size{
    
    [self setImage:[Utility imageWithImage:placeholder scaledToSize:size] forState:state];
    ...
    id <SDWebImageOperation> operation = [SDWebImageManager.sharedManager downloadImageWithURL:url options:SDWebImageRefreshCached progress:nil completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, BOOL finished, NSURL *imageURL) {
        ...
        dispatch_main_sync_safe(^{
            ...
            if (!sself) return;
            if (image) {
                [sself setImage:[Utility imageWithImage:image scaledToSize:size] forState:state];
            }
        });
    }];
