...
-(void)configureView{
    //ImageView
    UIImageView *imageView = [[UIImageView alloc] initWithFrame:CGRectMake(0,90,101*SCALE_W,35*SCALE_W)];
    ...
    //UITextField
    textFieldUser = [[UITextField alloc] initWithFrame:CGRectMake(10, imageView.bottom + 20, SCREEN_WIDTH - 60, 40)];
    textFieldUser.centerX = view.centerX;
    textFieldUser.borderStyle = UITextBorderStyleRoundedRect;
    textFieldUser.font = [Utility fontHelveticaNeueLight:[ProjectStyle getFormattedFontSize:18]];
    ...
    [textFieldUser addTarget:self action:@selector(textFieldEditingChanged:) forControlEvents:UIControlEventEditingChanged];
    [view addSubview:textFieldUser];
    
    //UITextFieldd
    textFieldPassword = [[UITextField alloc] initWithFrame:CGRectMake(10, textFieldUser.bottom + 20, SCREEN_WIDTH - 60, 40)];
    textFieldPassword.centerX = view.centerX;
    textFieldPassword.borderStyle = UITextBorderStyleRoundedRect;
    textFieldPassword.font = [Utility fontHelveticaNeueSemibold:[ProjectStyle getFormattedFontSize:18]];
    ...
    [textFieldPassword addTarget:self action:@selector(textFieldEditingChanged:) forControlEvents:UIControlEventEditingChanged];
    textFieldPassword.delegate = self;
    [view addSubview:textFieldPassword];
    
    btnLogin = [Helper getThemedBlueButtonWithTitleForNormalState:@"Login" andFrame:(CGRectMake(0, textFieldPassword.bottom + 20, textFieldPassword.width, 40))];
    ...
    UILabel *registerLabel = [[UILabel alloc] initWithFrame:CGRectMake(sideMargins, btnLogin.bottom + 25*SCALE_H,self.view.width - (sideMargins*2.0), 20)];
    registerLabel.textColor = [UIColor whiteColor];
    [registerLabel setFont:[Utility fontHelveticaNeue:[ProjectStyle getFormattedFontSize:13]]];
    registerLabel.textAlignment = NSTextAlignmentCenter;
    registerLabel.numberOfLines = 0;
    NSString *fullString = [NSString stringWithFormat:@"%@ %@",@"Don’t have an account?", NSLocalizedString(@"btn_register", nil)];
    NSString *registerString = [NSString stringWithFormat:@"%@", NSLocalizedString(@"btn_register", nil)];
    NSRange rangeLabelRegister = [fullString rangeOfString:registerString];
    NSMutableAttributedString *textRegisterString = [[NSMutableAttributedString alloc] initWithString:fullString];
    [textRegisterString setAttributes:@{NSFontAttributeName:[Utility fontHelveticaNeueBold:[ProjectStyle getFormattedFontSize:13]]}
                                range:rangeLabelRegister];
    [textRegisterString addAttribute:NSUnderlineStyleAttributeName value:[NSNumber numberWithInteger:NSUnderlineStyleSingle]
                               range:rangeLabelRegister];
    ...
    [btnLogin setBackgroundColor:[UIColor colorWithRed:155/255.0f green:155/255.0f blue:155/255.0f alpha:1.0]];
...
-(void)textFieldEditingChanged :(UITextField *) textField{
    if ([textFieldUser.text isEqualToString:@""] || [textFieldPassword.text isEqualToString:@""]){
